...
import scan_student_result_page_game1 as ssrp
...
class ScanSelectedQuestionsDisplayGame1:
    ...
    def on_mouse_click(self,event):
        print("on mouse click...GAME 1")
        print("counter as soon as clicked", self.counter)
        if self.counter == 0:
            self.actual_index =  len(self.button_widgets)-1
        else:
            self.actual_index = self.counter - 1
        
        print("indexes clicked..counter and actual index",self.highlight_index, self.actual_index,self.counter)
        ...
        if self.actual_index == len(self.scan_list) -2:
            ...
            if self.current_index >= len(self.exercise_questions):
                print("OVER")
                ssrp.ScanStudentResultPageGame1(self.master, self.header_frame, self.display_frame,self.student_id, self.teacher_id, self.exercise_name,self.category, self.exercise_questions, self.game1_final_result)
                
            else:
                ScanSelectedQuestionsDisplayGame1(self.master,self.header_frame,self.display_frame,self.student_id,self.teacher_id,self.exercise_name,self.category, self.exercise_questions, self.current_index, self.game1_final_result)

        else:
            val = self.scan_list[self.actual_index]
            print("my val", self.actual_index, val)
...
def main():
    ...
    ex_questions = [[1,1,"gave"],[2,2,"hope"]]
    my_result = [] 
    ScanSelectedQuestionsDisplayGame1(root,header_frame,display_frame,1,1,"Scramble", "Alphabet Scramble", ex_questions, 0, my_result)
    root.mainloop()
