...
class ScanStudentResultPageGame1:
    ...
    def calculate_score(self):
        ...
        for row in self.final_result:
            print("row", row.strip())
            print("original", self.exercise_questions[i][2])
            if row.strip() == self.exercise_questions[i][2]:
    ...
    def insert_result_data(self):
        ...
        for row in self.final_result:
            print("row", row.strip())
            db.insert_result_for_student(self.student_id,self.teacher_id,self.exercise_id,self.exercise_questions[i][0],self.exercise_questions[i][1],row.strip(),self.exercise_questions[i][2])
            print("original", self.exercise_questions[i][2])     
            i=i+1
...
""" def main():
    ...
    my_result = ["how ","I am fine "] 
    ScanStudentResultPageGame1(root,header_frame,display_frame,1,1,"GAME 2 EXERCISE","Alphabet Scramble", ex_questions,my_result)
    root.mainloop()
