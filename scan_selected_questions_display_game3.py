...
import scan_student_result_page_game3 as ssrp
...
class ScanSelectedQuestionsDisplayGame3:
    def __init__(self, master, header_frame, display_frame, student_id, teacher_id, selected_exercise_name, category,exercise_questions, current_index,result ):
        ...
        self.game3_final_result = result
        self.game3_result = ""
        self.game3_result_number = 0
        self.category = category

        self.scan_list = self.get_scan_list(self.exercise_questions[self.current_index])

        self.title = self.exercise_questions[self.current_index][2]
        ...
        self.game3_result_header_label = tk.Label(self.display_frame,  font=("Arial",30,"bold"),width=30, bg=TITLEBG,fg=TITLEFG)
    ...
    def create_buttons(self):
        ...
        rowval= rowval + 1
        self.game3_result_header_label.grid(row=rowval, column = 0, columnspan=2, pady=50)            
    ...
    def on_mouse_click(self,event):
        ...
        if self.actual_index == len(self.scan_list) -2:
            ...
            self.game3_result=""
            self.game3_result_number=0
            self.game3_result_header_label.configure(text=self.game3_result)
        elif self.actual_index == len(self.scan_list) - 1:
            print("submit")
            self.game3_final_result.append(self.game3_result_number)
            print("my results.....",self.game3_result,self.game3_final_result)
            ...
            if self.current_index >= len(self.exercise_questions):
                print("OVER")
                ssrp.ScanStudentResultPageGame3(self.master, self.header_frame, self.display_frame,self.student_id, self.teacher_id, self.exercise_name,self.category, self.exercise_questions, self.game3_final_result)
            else:
                ScanSelectedQuestionsDisplayGame3(self.master,self.header_frame,self.display_frame,self.student_id,self.teacher_id,self.exercise_name, self.category,self.exercise_questions, self.current_index, self.game3_final_result)

        else:
            val = self.scan_list[self.actual_index]
            self.game3_result = val
            self.game3_result_number = self.actual_index + 1
            print("result.....",self.scan_list, self.game3_result,self.actual_index,self.scan_list[self.actual_index])
            self.game3_result_header_label.configure(text=self.game3_result)
...
def main():
    ...
    ex_questions = [[1,"Question 1?", "opt 1","opt 2","opt 3","opt 4",2],[1,"Question 2?", "opti 1","opti 2","opti 3","opti 4",3]]
    my_result = [] 
    ScanSelectedQuestionsDisplayGame3(root,header_frame,display_frame,1,1,"Scramble",1, "Alphabet Scramble", ex_questions, 0, my_result)
    root.mainloop()
